...
type LeastBusyEdgeStrategy struct {
}

func (l *LeastBusyEdgeStrategy) selectDeploymentNode(ctx *StrategyContext, name string) *NodeConnection {
...
func (l *LeastBusyEdgeStrategy) SelectNode(ctx *StrategyContext, name string) (*NodeSelectionResult, error) {
	ctx.Mutex.RLock()
	defer ctx.Mutex.RUnlock()
